...
1. **Create the tool factory** in `packages/server/src/lib/tools/`:

```typescript
// packages/server/src/lib/tools/my-tool.tool.ts
import { tool } from 'ai';
import { z } from 'zod';
...
|---|---|---|
| `use_skill` | `createUseSkillTool(dir)` | Reads and executes a named skill file |
| `get_time` | `getTimeTool()` | Returns current UTC time |

## Location

```
packages/server/src/lib/tools/     ← tool factories
